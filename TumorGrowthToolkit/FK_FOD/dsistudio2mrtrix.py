...
def dsistudio_to_mrtrix(fib_file, sh_mif):
    ...
    valid_odfs = []
    flat_mask = (fibmat["fa0"].squeeze() > 0).flatten(order="F")
    ...
    affine = np.eye(4)
    affine[:3, 3] = -np.array(odf4d.shape[:3]) / 2 
    odf4d_img = nb.Nifti1Image(odf4d, affine=affine)
...
if __name__ == "__main__":
    dsistudio_to_mrtrix("HCP1065.1.25mm.odf.fib", "HCP1065.1.25mm.odf.nii.gz")