...
class FK_Fixel_n_Orig_Solver(FK_Solver):
    ...
    def FK_update_DTI_style(self, A,
                            Dpx, Dmx, Dpy, Dmy, Dpz, Dmz,
                            D_FK_mx, D_FK_px, D_FK_my, D_FK_py, D_FK_mz, D_FK_pz,
        ...
        SP_x = D_FK_px* diffs['x'] - D_FK_mx* diffs['minus_x']
        SP_y = D_FK_py* diffs['y'] - D_FK_my* diffs['minus_y']
        SP_z = D_FK_pz* diffs['z'] - D_FK_mz* diffs['minus_z']
        ...
        diff_A = (SP + SP_x + SP_y + SP_z + f*np.multiply(A,1-A)) * dt
        A += diff_A
        return A
    ...
    def solve(self, doPlot = False):
        ...
        cropped_GM = sGM[min_coords[0]:max_coords[0], min_coords[1]:max_coords[1], min_coords[2]:max_coords[2]]
        cropped_WM = sWM[min_coords[0]:max_coords[0], min_coords[1]:max_coords[1], min_coords[2]:max_coords[2]]

        D_FK_style = self.get_D(cropped_WM, cropped_GM, th_matter, Dw, self.params['RatioDw_Dg'])
        ...
        try:
            ...
            D_FK_px = D_FK_style['D_plus_x'] / (dx * dx)
            D_FK_mx = D_FK_style['D_minus_x'] / (dx * dx)
            D_FK_py = D_FK_style['D_plus_y'] / (dy * dy)
            D_FK_my = D_FK_style['D_minus_y'] / (dy * dy)
            D_FK_pz = D_FK_style['D_plus_z'] / (dz * dz)
            D_FK_mz = D_FK_style['D_minus_z'] / (dz * dz)
