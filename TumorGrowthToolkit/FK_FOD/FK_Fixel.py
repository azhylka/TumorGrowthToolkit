...
class FK_Fixel_Solver(FK_Solver):
    ...
    def get_D_from_Fixel_DTI_style(self, fixel_dir, num_fixels=3):
        D = {}
    ...
    def FK_update_DTI_style(self, A, D, f, dt, dx, dy, dz, fixel_dir):
        ...
        for fixel_idx in range(3):
            ...
            SP_y = 1/(dy*dy) * (D[f"D{fixel_idx}_plus_y"]* (np.roll(A,1,axis=1) - A) - D[f"D{fixel_idx}_minus_y"]* (A - np.roll(A,-1,axis=1)) )
            SP_z = 1/(dz*dz) * (D[f"D{fixel_idx}_plus_z"]* (np.roll(A,1,axis=2) - A) - D[f"D{fixel_idx}_minus_z"]* (A - np.roll(A,-1,axis=2)) )

            SP += fixel_ratios[..., fixel_idx] * (SP_x + SP_y + SP_z)
    ...
    def solve(self, doPlot = False):
        ...
        D_domain = self.get_D_from_Fixel_DTI_style(self.params['fixel_dir'])
